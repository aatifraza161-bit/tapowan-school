...
async function runBrowserScrape() {
  console.log('🚀 INITIATING HEAVY ARTILLERY BROWSER SCRAPE (FULL BYPASS)...');
  ...
  // Fake user agent to look like a standard Windows Chrome user
  await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36');

  let totalNewInserted = 0;

  for (const mapping of PW_MAPPINGS) {
    console.log(`\n=== Browser scraping PW videos for ${mapping.className} ===`);
    const newUniqueVideos = [];
    
    try {
      const searchUrl = `https://www.youtube.com/results?search_query=${encodeURIComponent(mapping.query)}&sp=EgIYQQ%253D%253D`; // sp limits to short videos if possible, or just raw search
      ...
      for (const v of videos) {
        if (!existingGlobalSet.has(v.yt)) {
          ...
          newUniqueVideos.push(v);
        }
      }
      
      console.log(`✅ Extracted ${newUniqueVideos.length} NEW unique videos directly from DOM for ${mapping.className}.`);
    } catch (e) {
    ...
    console.log(`  🎉 [${mapping.className}] Inserted ${count} videos.`);
  }

  await browser.close();
  console.log(`\n✅ BROWSER SYNC COMPLETE! Exactly ${totalNewInserted} New PW Videos Added to respective classes.`);
}

runBrowserScrape().then(() => process.exit(0)).catch(e => { console.error(e); process.exit(1); });
