...
// 1. Remove duplicate `deleteClassDayTimetable` and define `saveStore`
const regexDel = /window\.deleteClassDayTimetable = function[\s\S]*?;\s*/g;
code = code.replace(regexDel, '');
...
// 2. Fix applySmartTimetable
const applyOld = /function applySmartTimetable\(\) \{[\s\S]*?\}\s*\n/g;
const applyNew = `
...
code = code.replace(applyOld, applyNew);

fs.writeFileSync('public/app.js', code);
console.log('Fixed applySmartTimetable and deleteClassDayTimetable.');
