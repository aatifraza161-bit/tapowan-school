...
const regex = /const history = \[\.\.\.dueHistory, \.\.\.feeHistory\][\s\S]*?const printBtns = refs\.studentProfileContent\.querySelectorAll\('\.print-fee-btn'\);/g;

const replacement = `const history = [...dueHistory, ...feeHistory]
      ...
      .map((f) => {
        ...
        return \`
        <div style="background:\${cardBg}; border-radius:var(--radius); margin-bottom:16px; border:1px solid \${cardBorder}; box-shadow:var(--shadow); position:relative; overflow:hidden; padding:24px; transition:all 0.3s var(--spring);">
...
console.log("Successfully styled the fee cards!");
