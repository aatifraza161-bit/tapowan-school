...
desktop_path = os.path.join(os.path.expanduser('~'), 'Desktop', 'Students_Without_Photos.csv')

conn = sqlite3.connect(db_path)
cursor = conn.cursor()

cursor.execute("SELECT admissionNo, rollNo, fullName, className, phone, fatherName, motherName FROM students WHERE photo IS NULL OR photo = ''")
