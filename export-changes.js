...
const rawData = `Updated PAWAN SOREN: Father(SATISH SOREN -> SATISH SOREN), Mother(TALOMUNI DEVI -> TALOMUNI DEVI)
...
const rows = [
  ['Student Name', 'Old Father Name', 'New Father Name', 'Old Mother Name', 'New Mother Name']
];

for (const line of rawData.trim().split('\n')) {
  // Format: Updated NAME: Father(OLD -> NEW), Mother(OLD -> NEW)
  const match = line.match(/^Updated (.*?): Father\((.*?) -> (.*?)\), Mother\((.*?) -> (.*?)\)$/);
  if (match) {
    const studentName = match[1];
    let oldFather = match[2];
    let newFather = match[3];
    let oldMother = match[4];
    let newMother = match[5];
    
    // Note: If oldFather/Mother hasn't changed, we'll still show the correct updated parentName side-effect
    rows.push([studentName, oldFather, newFather, oldMother, newMother]);
  }
}
...
const outPath = 'C:\\\\Users\\\\Admin\\\\Desktop\\\\School Work\\\\Apaar data\\\\Updated_Students_List_v2.xlsx';
