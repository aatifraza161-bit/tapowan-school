...
const remoteFeeClient = createClient({
...
async function run() {
  ...
  const tablesResult = await localClient.execute("SELECT name, sql FROM sqlite_master WHERE type='table' AND name IN ('fees', 'feeStructures', 'dueManagement')");
  const tables = tablesResult.rows;
  
  for (const tableObj of tables) {
    const table = tableObj.name;
    const createSql = tableObj.sql;
    console.log(`\nMigrating table: ${table} to FEE DB`);
    
    // Create remote table if missing
    if (createSql) {
      await remoteFeeClient.execute(createSql.replace('CREATE TABLE', 'CREATE TABLE IF NOT EXISTS'));
      console.log(`  Created remote table ${table}`);
    }
    
    // Clear remote table
    await remoteFeeClient.execute(`DELETE FROM ${table}`);
    ...
    for (let i = 0; i < batch.length; i += 50) {
      ...
      await remoteFeeClient.batch(chunk, 'write');
      console.log(`  Inserted rows ${i + 1} to ${Math.min(i + 50, batch.length)}`);
    }
  }
  
  console.log("\nFee Migration completed successfully!");
}
